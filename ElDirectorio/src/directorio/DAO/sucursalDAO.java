...
public class sucursalDAO {
	...
	public double getPosx(String address) {
		if (db.isOpen() == false) {
			openDB();
		}
		String query = "Select * from Office where Address = '" + address
				+ "';";
		Cursor c = db.rawQuery(query, null);
		double resultado = c.getDouble(6);
		c.close();
		db.close();
		return resultado;
	}

	public double getPosy(String address) {
		if (db.isOpen() == false) {
			openDB();
		}
		String query = "Select * from Office where Address = '" + address
				+ "';";
		Cursor c = db.rawQuery(query, null);
		double resultado = c.getDouble(7);
		c.close();
		db.close();
		return resultado;
	}
